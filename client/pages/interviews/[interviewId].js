...
const InterviewShow = ({ currentUser, interview, participantsList }) => {
  useEffect(() => {
    if (!currentUser || interview.length <= 0) Router.push("/auth/signup");
  }, [currentUser]);
...
InterviewShow.getInitialProps = async (context, client, currentUser) => {
  if (currentUser) {
  ...
  return { interview: {}, participantsList: {} };
};
